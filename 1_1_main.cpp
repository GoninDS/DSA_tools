 ...
 * Imprime un mensaje de error y los datos de la tarea del ordenador antes de finalizar el programa.
 *
 * @param signal La señal recibida.
 */
void signalHandler(int signal) {
    if (signal == SIGABRT || signal == SIGSEGV || signal == SIGILL || signal == SIGFPE) {
        std::cerr << "Error crítico detectado durante el ordenamiento. Posible causa: ";

        switch (signal) {
            case SIGABRT: std::cerr << "Llamada a abort()"; break;
            case SIGSEGV: std::cerr << "Violación de segmento (acceso inválido a memoria)"; break;
            case SIGILL: std::cerr << "Instrucción ilegal"; break;
            case SIGFPE: std::cerr << "Error de punto flotante (división por cero, etc.)"; break;
            default: std::cerr << "Señal desconocida";
        }
 ...
 *
 * @param ordenador Referencia constante al objeto Ordenador que contiene el método de ordenamiento.
 * @param funcionOrdenamiento Puntero a la función de ordenamiento miembro de Ordenador.
 * @param nombreMetodo Nombre del método de ordenamiento, usado para imprimir mensajes.
 */
void probarProgramacionDefensiva(const Ordenador& ordenador, void (Ordenador::*funcionOrdenamiento)(int*, int) const, const std::string& nombreMetodo) {
    ...
    int arreglo[] = {0,0,0};
    int tamano = 3;
    std::cout << "----Realizando Pruebas defensivas sobre " << nombreMetodo << "----" << std::endl;
    ...
    std::cout << "---- ✓ Pruebas defensivas sobre " << nombreMetodo << " completadas sin errores.----" << std::endl;
}
